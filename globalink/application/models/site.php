...
Class Site extends CI_Model {
	...
	public function getjobbytype2($id,$date){
		$query=$this->db->query("SELECT b.name,SUM(a.qty) as qty FROM job_order a,products b,transaction c WHERE a.job_type=b.id AND a.order_no=c.order_no AND a.dept='$id' AND a.date='$date'  group by a.job_type");
		return $query->result_array();
		
	}
	public function getDICountSummary2($from,$to){
		$query=$this->db->query("SELECT SUM(a.countQty) as qty,b.name FROM dicount a,products b WHERE a.countType=b.id AND a.countDate BETWEEN '$from' AND '$to'");
		return $query->result_array();
	}
	public function getDICountSummary($date){
		$query=$this->db->query("SELECT SUM(a.countQty) as qty,b.name FROM dicount a,products b WHERE a.countType=b.id AND a.countDate='$date'");
		return $query->result_array();
	}
	public function insertDICount($qty,$type,$date){
		$data=array('countDate'=>$date,'countQty'=>$qty,'countType'=>$type);
		return $this->db->insert('dicount',$data);
	}
	public function deletDICount($id){
		$this->db->where('countID',$id);
		return $this->db->delete('dicount');
	}
	public function getAllDICount(){
		$query=$this->db->query("SELECT * FROM dicount a,products b WHERE a.countType=b.id AND b.dept='4'");
		return $query->result_array();
	}
	public function getjobbytype2search($id,$from,$to){
		$query=$this->db->query("SELECT b.name,SUM(a.qty) as qty FROM job_order a,products b,transaction c WHERE a.job_type=b.id AND a.order_no=c.order_no AND a.dept='$id' AND a.date BETWEEN '$from' AND '$to' group by a.job_type");
		return $query->result_array();
		
	}
	public function CheckAssignedJob($assignedTo,$id,$order_no){
		$this->db->where('adminID',$assignedTo);
		$this->db->where('jobID',$id);
		$this->db->where('orderNo',$order_no);
		$row=$this->db->get('stockhistory')->row_array();
		return $row["stockID"];
	}
	...
	public function getMyStockHistory($adminID){
		$query=$this->db->query("SELECT * FROM stockhistory a,job_order_tmp b WHERE a.jobID=b.id AND a.adminID='$adminID'");
		return $query->result_array();
	}
	...
	public function getlargeformatjobsx(){
		$query=$this->db->query("SELECT a.qty, a.order_no,a.id,a.cust_name,a.date,a.description,a.s1,a.s2 FROM job_order_tmp a,transaction b WHERE a.order_no=b.order_no AND  a.DATE <= ( NOW( )) AND a.DATE >= ( NOW( ) - INTERVAL 2880 MINUTE ) AND (a.dept='1' or a.dept='3') order by a.date DESC ");
		return $query->result_array();
	}
	public function deleteMyStockHistory($check){
		$this->db->where('stockID',$check);
		return $this->db->delete('stockhistory');
	}
	...
	public function getjobdetailsx($id){
		$this->db->where('id',$id);
	...
	public function staffstockDetails($id){
	 ...
	 public function getExpenses($date,$user_id){
        $query=$this->db->query("SELECT * FROM expenses WHERE addedDate='$date' AND expensesBy='$user_id'");
        return $query->result_array();
    }
	public function getFloats($date,$user_id){
        $query=$this->db->query("SELECT * FROM floats WHERE addedDate='$date' AND floatBy='$user_id'");
        return $query->result_array();
    }
	public function getMyFloats($date){
        $query=$this->db->query("SELECT * FROM `floatexp` where floatExpDate='$date' order by floatExpdate,floatExpID ASC");
        return $query->result_array();
    }
	public function getMyFloats2($from,$to){
        $query=$this->db->query("SELECT * FROM `floatexp` where floatExpDate BETWEEN '$from' AND '$to' order by floatExpdate,floatExpID ASC");
        return $query->result_array();
    }
	public function getjobsbreakdown($jobOrderNo){
		$this->db->where('jobNo',$jobOrderNo);
	...
	{
    $this->db->where('floatID',$id);
    return $this->db->delete('floats');
	}
...
public function addExpensesx($expensesName,$expensesDesc,$expensesAmt,$expensesDate,$expensesBy,$jobNo,$refNo,$addedDate){
    $data=array('expenses'=>$expensesName,'expensesDesc'=>$expensesDesc,'expensesAmt'=>$expensesAmt,'expensesDate'=>$expensesDate,'expensesBy'=>$expensesBy,'jobNo'=>$jobNo,'expRefNo'=>$refNo,'addedDate'=>$addedDate);
    return $this->db->insert('expenses',$data);
}
public function insertexpfloat($expRefNo,$expensesDate,$expensesName,$expensesDesc,$expensesAmtDr,$expensesAmtCr,$expensesBy){
	$data=array('floatRefNo'=>$expRefNo,'floatExpDate'=>$expensesDate,'floatExp'=>$expensesName,'floatExpDesc'=>$expensesDesc,'floatExpDr'=>$expensesAmtDr,'floatExpCr'=>$expensesAmtCr,'floatBy'=>$expensesBy);
	return $this->db->insert('floatexp',$data);
}
public function deletefloatexp($refNo){
	$this->db->where('floatRefNo',$refNo);
	return $this->db->delete('floatexp');
}
public function addFloat($floatNo,$floatRefNo,$floatName,$floatDesc,$floatAmt,$floatDate,$floatBy,$addedDate){
	$data=array('floatNo'=>$floatNo,'floatRefNo'=>$floatRefNo,'floatName'=>$floatName,'floatDesc'=>$floatDesc,'floatAmt'=>$floatAmt,'floatDate'=>$floatDate,'floatBy'=>$floatBy,'addedDate'=>$addedDate);
	return $this->db->insert('floats',$data);
}
public function getExpenses2($from,$to,$user_id){
	...
	public function getFloats2($from,$to,$user_id){
        $query=$this->db->query("SELECT * FROM floats WHERE floatDate BETWEEN '$from' AND '$to' AND floatBy='$user_id'");
        return $query->result_array();
    }
	...
	public function checkEmail($email){
		...
		public function enterEnqxy16($enqNo,$name,$phone,$item,$amount,$date,$entered_by,$enqQty,$enqPrice,$operator){
			$data=array('enqNo'=>$enqNo,'enqName'=>$name,'enqPhone'=>$phone,'enqItem'=>$item,'enqAmount'=>$amount,'enqDate'=>$date,'enteredby'=>$entered_by,'enqQty'=>$enqQty,'enqPrice'=>$enqPrice,'processed'=>$operator);
			return $this->db->insert('enquiries',$data);
		}
        ...
        public function authenticate22($username,$password){
			...
			return $result;
			//error_log("Db error:".print_r($result,TRUE));
		...
		public function loadJobsX($dept,$date,$adminID){
			$this->db->where('date',$date);
			//$this->db->where('dept',$dept);
			$this->db->where('cashier',$adminID);
			return $this->db->get('job_order')->result_array();
		...
		public function loadBF222(){
			$date=date('Y-m-d');
			$query=$this->db->query("SELECT * FROM transaction t,job_order j,admin a WHERE t.order_no=j.order_no AND j.generated_by=a.id AND j.balance > '0' AND j.date <'$date' AND j.dept='5' GROUP BY j.order_no ORDER BY j.date DESC");
			return $query->result_array();
			
		}
		public function loadDiscount(){
			$date=date('Y-m-d');
			$query=$this->db->query("SELECT * FROM transaction t,job_order j,admin a WHERE t.order_no=j.order_no AND j.generated_by=a.id AND j.balance > '0' AND j.discount >'0' GROUP BY j.order_no ORDER BY j.date DESC");
			return $query->result_array();
		}
		public function updatecleared($order_no){
			$this->db->where('order_no',$order_no);
			$data=array('balance'=>'0');
			return $this->db->update('job_order',$data);
		}
		public function updatediscountcleared($order_no,$balance,$newadvance){
			$this->db->where('order_no',$order_no);
			$data=array('balance'=>$balance,'discount'=>0,'advance'=>$newadvance);
			return $this->db->update('job_order',$data);
		}
		public function insertclearance($date,$order_no,$adminID,$balance){
			$data=array('clearDate'=>$date,'clearOrderNo'=>$order_no,'clearedBy'=>$adminID,'clearedAmount'=>$balance);
			return $this->db->insert('jobscleared',$data);
		}
		...
		public function retrieveJOrder($orderNo){
			$this->db->where('order_no',$orderNo);
			return $this->db->get('job_order')->result_array();
		}
		public function updateJStatus($order_no,$status){
			$this->db->where('order_no',$order_no);
			$data=array('jobStatus'=>$status);
			return $this->db->update('job_order',$data);
		}
		public function retrieveMyJobOrderFM($date){
			$query=$this->db->query("SELECT * FROM job_order a,products b WHERE a.job_type=b.id AND a.date='$date'");
			return $query->result_array();	
		...
		public function updateJobOrderNew($order_no,$amount,$balance,$remarks,$discount){
			$data=array('advance'=>$amount,'balance'=>$balance,'remarks'=>$remarks,'discount'=>$discount);
			$this->db->where('order_no',$order_no);
			return $this->db->update('job_order',$data);
		}
		...
		public function loadGeneralStaff($role){			
			$this->db->where('user_type',$role);
			return $this->db->get('admin')->result_array();
		}
		...
		public function searchJobOrderFM($from,$to){
			$query=$this->db->query("SELECT * FROM job_order a,products b WHERE a.job_type=b.id  AND date BETWEEN '$from' AND '$to'");
			return $query->result_array();
		}
