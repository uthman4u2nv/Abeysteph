...
Class Site extends CI_Model {
	...
	public function getalltimeissuance(){
		$query=$this->db->query("SELECT SUM(a.issuedqty) as qty,b.name FROM `platesissuance` a,products2 b WHERE a.productID=b.id GROUP BY a.productID");
		return $query->result_array();
	}
    
	public function getIssuedhistory(){
		$query=$this->db->query("SELECT * FROM platesissuance a,admin b,products2 c WHERE a.issuedBy=b.id AND a.productID=c.id");
		return $query->result_array();
	}
	public function insertissuance($date,$qty,$product,$adminID){
		$data=array('issueDate'=>$date,'issuedQty'=>$qty,'productID'=>$product,'issuedBy'=>$adminID);
		return $this->db->insert('platesissuance',$data);
	}
	public function deleteissuance($id){
		$this->db->where('issueID',$id);
		return $this->db->delete('platesissuance');
	}
	public function checkEmail($email){
		...
		public function insertNewProductxx($name,$dept,$price,$status){
			$data=array('name'=>$name,'price'=>$price,'dept'=>$dept,'status'=>$status);
			return $this->db->insert('products2',$data);
		...
		public function retrieveTransactiony($date){
			$query=$this->db->query("SELECT * FROM transaction b,job_order a,depts c,products d,admin e WHERE a.order_no=b.order_no AND b.entered_by=e.id AND a.dept=c.id AND a.job_type=d.id AND a.date='$date' AND b.type='0' AND (b.status='Paid' OR b.status='Fully Paid')  ORDER BY a.id DESC  ");
			return $query->result_array();
		}
		...
		public function getProduct2($id){
			$this->db->where('id',$id);
			return $this->db->get('products2')->result_array();
		}
		...
		public function updateproductxx($id,$name,$price,$status){
			$data=array('name'=>$name,'price'=>$price,'status'=>$status);
			$this->db->where('id',$id);
			return $this->db->update('products2',$data);
		}
		...
		public function updatePayrollStaff($id,$name,$dob,$phone,$email,$sex,$address,$salary,$status,$username,$password,$bank,$accountName,$accountNo,$status1){
